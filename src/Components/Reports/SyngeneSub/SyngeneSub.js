...
const SyngeneSub = () => {
  ...
  useEffect(() => {
    const fetchSubData = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/syngene-reports/getSubData', {
          headers: {
        ...
        // setFilteredEntryExitData(response.data.data)
        ...
        if (error.response && error.response.status === 401) {
          ...
          setFilteredEntryExitData([]); // Set empty array if no data found
        } else {
  ...
  const handleDownload = () => {
    if (filteredEntryExitData.length > 0) {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, 'SubData');
      XLSX.writeFile(workbook, 'subData.csv');
    }
  };
  ...
  const handleSearchChange = (event) => {
    ...
    setSearchName(query);
    const filteredData = subData.filter(entry =>
  ...
  return (
    <div className='syngene-entry-exit custom-scrollbar'>
      ...
      <div className='syngene-entry-exit-heading-div'>
        <div className='syngene-entry-exit-heading-div-i2'><h1 className='syngene-entry-exit-heading'>Procedure for loading and assembling the sub-bags</h1></div>
      </div>
      ...
      <div className="pagination">
        <button className='arrow_button' onClick={() => handlePageChange(1)}>&laquo;</button>
        {Array.from({ length: totalPages }, (_, i) => (
          <button key={i} className={currentPage === i + 1 ? "active" : ""} onClick={() => handlePageChange(i + 1)}>{i + 1}</button>
        ))}
        <button className='arrow_button' onClick={() => handlePageChange(totalPages)}>&raquo;</button>
      </div>
    </div>
  )
}

export default SyngeneSub
