...
import './SyngeneEntryExit.css';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faDownload } from '@fortawesome/free-solid-svg-icons';
import axios from 'axios';
import * as XLSX from 'xlsx';

const SyngeneEntryExit = () => {
    ...
    useEffect(() => {
        const fetchEntryExitData = async () => {
            try {
                ...
                const response = await axios.get('http://localhost:5000/syngene-reports/getEntryExitData', {
                    headers: {
    ...
    const handleDownload = () => {
        if (filteredEntryExitData.length > 0) {
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, 'EntryExitData');
            XLSX.writeFile(workbook, 'entry_exit_data.csv');
        }
    };
    ...
    const handleSearchChange = (event) => {
        ...
        setSearchName(query);
        const filteredData = entryExitData.filter(entry =>
    ...
    return (
        <div className='syngene-entry-exit custom-scrollbar'>
            ...
            <div className='syngene-entry-exit-heading-div'>
                <div className='syngene-entry-exit-heading-div-i2'><h1 className='syngene-entry-exit-heading'>Entry and exit procedure for BMP 1 - Primary gowning</h1></div>
            </div>
...
export default SyngeneEntryExit;
